...
namespace Libgame.Utils.Checksums
{
    ...
    /// <summary>
    /// CRC-32
    /// Code from GZIP project: http://git.savannah.gnu.org/cgit/gzip.git/tree/util.c
    /// </summary>
    public static class Crc32
    {
        ...
        /// <summary>
        /// Calculate the CRC32 from the stream.
        /// </summary>
        /// <param name="stream">Stream to read data.</param>
        /// <param name="length">Number of bytes to read.</param>
        public static uint Run(DataStream stream, uint length)
        {
            uint crc = 0xffffffff;
            while (length-- > 0)
                crc = Table[((int)crc ^ stream.ReadByte()) & 0xff] ^ (crc >> 8);
            return ~crc;
        }
    }
}
